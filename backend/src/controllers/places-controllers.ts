...
export const getPlaceById = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!place) {
            return next(new AppError(404, 'Could not find a place for the provided id'));
        }
...
export const createPlace = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user) {
        return next(new AppError(404, 'Could not find user for provided id'));
    }
...
export const updatePlace = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    ...
    if (!req.userData || place.creatorId.toString() !== req.userData.userId.toString()) {
        return next(new AppError(401, 'You are not allowed to edit this place.'));
    }
...
export const deletePlace = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const placeId = req.params.pid;
